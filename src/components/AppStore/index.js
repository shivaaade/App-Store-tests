...
class AppStore extends Component {
  state = {tabsId: tabsList[0].tabId, newAppsList: appsList, userInput: ''}

  filteredApps = () => {
    const {tabsId, newAppsList} = this.state
    const filterItem = newAppsList.filter(each => each.category === tabsId)
    return filterItem
  }

  finalApps = tabId => {
    this.setState({tabsId: tabId})
  }
  ...
  render() {
    const {tabsId, newAppsList, userInput} = this.state
    const filterApps = this.filteredApps()
    const finalFilter = filterApps.filter(each =>
      each.appName.toLowerCase().includes(userInput.toLowerCase()),
    )
    return (
      <div>
        <h1>App Store</h1>
        <input onChange={this.userInputType} type="search" />
        <img
          alt="search icon"
